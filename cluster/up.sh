...
if [[ $image == $KUBERNETES_IMAGE ]]; then
    ...
    $gocli scp /etc/kubernetes/admin.conf - > ./cluster/$KUBEVIRT_PROVIDER/.kubeconfig
    export KUBECONFIG=./cluster/$KUBEVIRT_PROVIDER/.kubeconfig
    ...
    $gocli scp /etc/origin/master/admin.kubeconfig - > ./cluster/$KUBEVIRT_PROVIDER/.kubeconfig
